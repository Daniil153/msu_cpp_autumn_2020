...
public:
    ...
    BigInt(const std::string &s, size_t ssize=0) {
        ...
        data = new unsigned int [new_size];
        int count = 0;
        for (int i = (int)size; i > 0; i -= 8) {
            if (i < 8) {
                data[count] = (atoi (s.substr (t, i-t).c_str()));
            } else {
                data[count] = (atoi (s.substr (i-8, 8).c_str()));
            }
            count++;
        }
    }
    ...
    friend std::ostream& operator<<(std::ostream& out, BigInt& m);
};
std::ostream& operator<<(std::ostream& out, BigInt& m) {
    if (m.sign == -1) {
        out << '-';
    }
