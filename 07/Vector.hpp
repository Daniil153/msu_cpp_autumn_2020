...
public:
    ...
    Iterator<value_type>& operator +(int n) const {
...
public:
    ...
    Vector (const Vector& v) {
        size_ = v.size_;
        mem_size = v.mem_size;
        data = allocator.allocate(mem_size);
        for (size_t i = 0; i < size_; ++i)
            allocator.create(data + i, std::move(v.data[i]));
    }
    Vector (Vector&& v) noexcept {
        ...
        data = allocator.allocate(mem_size);
        for (size_t i = 0; i < size_; ++i)
            allocator.create(data + i, v.data[i]);
        v.size_ = 0;
        v.mem_size = 0;
    }
    ...
    void push_back(const value_type& data_) {
        ...
        allocator.create(data + size_, data_);
        ++size_;
    }
    template <class... Args>
    ...
    void reserve(size_t count) {
        if (count > mem_size) {
            ...
            for (size_t i = 0; i < size_; ++i){
                allocator.create(tmp + i, std::move(data[i]));
                allocator.destroy(data + i);
            }
